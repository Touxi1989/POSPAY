...
import com.telpo.solab.iso8583.util.Bcd;
...
public class IsoValue<T> implements Cloneable {
        ...
        if (type == IsoType.LLVAR || type == IsoType.LLLVAR) {
            ...
            if (custom == null) {
                if (value instanceof byte[]) {
                    ...
                    length = value.toString().length();// luyq modify
                    // llbin时长度根据字符串值
                }
            } else if (custom instanceof CustomBinaryField) {
                // length =
                // ((CustomBinaryField<T>)custom).encodeBinaryField(value).length;
                // luyq modify 改为按实际元素长度获取
                CompositeField v1 = (CompositeField) value;
                length = 0;
                for (IsoValue<?> v : v1.getValues()) {
                    length += v.getLength();
                }
        ...
        if (type == IsoType.NUMERIC || type == IsoType.AMOUNT) {
            ...
            if (value instanceof byte[]) {
                ...
                final String _s = encoder == null ? value.toString() : encoder
                        .encodeField(value);
                return (_s.length() % 2 == 1) ? String.format("%s0", _s) : _s;
            }
        }
     ...
     * @param binary              Specifies whether the value should be written in binary or
     *                            text format.
    ...
    public void write(final OutputStream outs, final boolean binary,
        ...
        if (binary
            ...
            if (value instanceof byte[]) {
                ...
                // luyq moidfy 数据长度不是偶数，右边加0
                byte[] binval = null;
                if (type == IsoType.LLBIN || type == IsoType.LLLBIN) {
