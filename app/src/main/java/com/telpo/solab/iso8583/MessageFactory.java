...
import com.telpo.solab.iso8583.parse.DateTimeParseInfo;
...
public class MessageFactory<T extends IsoMessage> {
    ...
    private String encoding = "gbk";//富友平台使用的是GBK编码
    ...
    /**
     * Tells the factory to create messages that encode their bitmaps in binary
     * format even when they're encoded as text. Has no effect on binary
     * messages.
     */
    ...
    /**
     * Returns true if the factory is set to create and parse bitmaps in binary
     * format when the messages are encoded as text.
     */
    ...
    /**
     * Specifies a map for custom field encoder/decoders. The keys are the field
     * numbers.
     */
    @SuppressWarnings("rawtypes")
    ...
    /**
     * Returns a custom field encoder/decoder for the specified field number, if
     * one is available.
     */
    ...
    /**
     * Tells the receiver to create and parse binary messages if the flag is
     * true. Default is false, that is, create and parse ASCII messages.
     */
     ...
     * terminator.
     *
     * @param value The ASCII value of the ETX character or -1 to indicate no
    ...
    /**
     * Creates a new message of the specified type, with optional trace and date
     * values as well as any other values specified in a message template. If
     * the factory is set to use binary messages, then the returned message will
     * be written using binary coding.
     *
    ...
    public T newMessage(int type) {
        ...
        m.setMessageHeadBcd(isMessageHeadBcd);// luyq modify 消息头部改BCD码
    ...
    /**
     * Sets the timezone for the specified FieldParseInfo, if it's indeed for
     * parsing dates.
     */
    public void setTimezoneForParseGuide(int messageType, int field, TimeZone tz) {
        ...
        System.out.println("Field " + field + " for message type "
                + messageType + " is not for dates, cannot set timezone");
    }

    /**
     * Creates a new message instance from the buffer, which must contain a
     * valid ISO8583 message. If the factory is set to use binary messages then
     * it will try to parse a binary message.
     *
     * @param buf             The byte buffer containing the message. Must not include the
     *                        length header.
     * @param isoHeaderLength The expected length of the ISO header, after which the message
     *                        type and the rest of the message must come.
     */
    public T parseMessage(byte[] buf, int isoHeaderLength)
        ...
        if (buf.length < minlength) {
            throw new ParseException(
                    "Insufficient buffer length, needs to be at least "
                            + minlength, 0);
        }
        final T m = createIsoMessage(isoHeaderLength > 0 ? new String(buf, 0,
                isoHeaderLength, encoding) : null);
        m.setCharacterEncoding(encoding);
        final int type;
        if (useBinary) {
       ...
       MLog.i(Integer.toHexString(type));
        ...
        if (index == null) {
            ...
            // type, new String(buf)));
//			System.out
//					.println("ISO8583 MessageFactory has no parsing guide for message type "
//							+ type + " [" + new String(buf) + "]");

            return null;
        }
        MLog.i("Parse each field 558");
        // First we check if the message contains fields not specified in the
        // parsing template
        boolean abandon = false;
        for (int i = 1; i < bs.length(); i++) {
            if (bs.get(i) && !index.contains(i + 1)) {
                // log.warn("ISO8583 MessageFactory cannot parse field {}: unspecified in parsing guide",
                // i+1);

//				System.out.println("ISO8583 MessageFactory cannot parse field "
//						+ (i + 1) + ": unspecified in parsing guide");

                abandon = true;
            }
        }
        MLog.i("Parse each field 576" + abandon);
        if (abandon) {
            return m;
        }
        ...
        if (useBinary) {
            for (Integer i : index) {
                ...
                MLog.i("FILE:" + i + " " + pos);
                if (bs.get(i - 1)) {
                    if (ignoreLast && pos >= buf.length
                            && i.intValue() == index.get(index.size() - 1)) {
                        // log.warn("Field {} is not really in the message even though it's in the bitmap",
                        // i);
                        System.out
                                .println("Field "
                                        + i
                                        + " is not really in the message even though it's in the bitmap");
                        bs.clear(i - 1);
                    } else {
                        ...
                        if (val.getType() == IsoType.DATE4 || val.getType() == IsoType.DATE10
                                || val.getType() == IsoType.DATE_EXP
                                || val.getType() == IsoType.TIME) {

                        } else if (val.getType() == IsoType.LLLBIN
                                || val.getType() == IsoType.LLBIN) {
                        }

                        m.setField(i, val);
                        if (val != null) {
                            if (val.getType() == IsoType.NUMERIC
                                ...
                                pos += (val.getLength() / 2)
                                        + (val.getLength() % 2);
                            } else if (val.getType() == IsoType.LLVAR
                                    || val.getType() == IsoType.LLLVAR) {
                                Object temp = val.getValue();
                                String str = (String) temp;
                                pos += ((String) temp).getBytes("gbk").length;
            ...
            for (Integer i : index) {
                ...
                if (bs.get(i - 1)) {
                    if (ignoreLast && pos >= buf.length
                        ...
                        m.setField(i, val);
                        // To get the correct next position, we need to get the
                        // number of bytes, not chars
                        pos += val.toString().getBytes(
                                fpi.getCharacterEncoding()).length;
                        if (val.getType() == IsoType.LLVAR
                                || val.getType() == IsoType.LLBIN) {
                            pos += 2;
                        } else if (val.getType() == IsoType.LLLVAR
     ...
     *
     * @param value A map where the keys are the message types and the values are
     *              the ISO headers.
     */
     ...
     * @param value The ISO header, or NULL to remove any headers for this message
     *              type.
     */
    ...
    /**
     * Adds a message template to the factory. If there was a template for the
     * same message type as the new one, it is overwritten.
     */
    ...
    /**
     * Returns the template for the specified message type. This allows
     * templates to be modified programatically.
     */
    ...
    /**
     * Invoke this method in case you want to freeze the configuration, making
     * message and parsing templates, as well as iso headers and custom fields,
     * immutable.
     */
    public void freeze() {
     ...
     * @param map  A map of FieldParseInfo instances, each of which define what
...
//				.format("ISO8583 MessageFactory adding parse map for type "
//						+ type + " with fields " + index));
