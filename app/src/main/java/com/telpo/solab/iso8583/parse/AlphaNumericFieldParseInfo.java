...
/**
 * This is the common abstract superclass to parse ALPHA and NUMERIC field
 * types.
 * 
	...
	public <T> IsoValue<?> parse(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid ALPHA/NUM field %d position %d", field, pos), pos);
		...
		try {
			...
			if (_v.length() != length) {
				_v = new String(buf, pos, buf.length - pos,
