	...
	public <T> IsoValue<Date> parse(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid DATE_EXP field %d position %d", field, pos), pos);
		}
		if (pos + 4 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient data for DATE_EXP field %d pos %d", field,
					pos), pos);
		}
		...
		if (forceStringDecoding) {
			cal.set(Calendar.YEAR,
					cal.get(Calendar.YEAR)
							...
							+ Integer.parseInt(new String(buf, pos, 2,
									getCharacterEncoding()), 10));
			cal.set(Calendar.MONTH, Integer.parseInt(new String(buf, pos + 2,
					2, getCharacterEncoding()), 10) - 1);
		} else {
			cal.set(Calendar.YEAR,
					cal.get(Calendar.YEAR) - (cal.get(Calendar.YEAR) % 100)
							+ ((buf[pos] - 48) * 10) + buf[pos + 1] - 48);
			cal.set(Calendar.MONTH, ((buf[pos + 2] - 48) * 10) + buf[pos + 3]
					- 49);
		}
		if (tz != null) {
			cal.setTimeZone(tz);
