...
/**
 * This class is used to parse a field from a message buffer. There are concrete
 * subclasses for each IsoType.
 * 
...
public abstract class FieldParseInfo {
	 ...
	 * the specified length. The length is only useful for ALPHA and NUMERIC
	 * types.
	 * 
	 * @param t
	 ...
	 * should be decoded using proper string conversion with the character
	 * encoding. Default is false, which means use the old behavior of decoding
	 * as ASCII.
	 */
	...
	/**
	 * Parses the character data from the buffer and returns the IsoValue with
	 * the correct data type in it.
	 * 
	 * @param field
	 ...
	 * @param custom
	 *            A CustomField to decode the field.
	 */
	...
	/**
	 * Parses binary data from the buffer, creating and returning an IsoValue of
	 * the configured type and length.
	 * 
	 * @param field
	 *            The field index, useful for error reporting.
	 * @param buf
	 ...
	 *            A CustomField to decode the field.
	 */
	public abstract <T> IsoValue<?> parseBinary(final int field, byte[] buf,
												int pos, CustomField<T> custom) throws ParseException,
			UnsupportedEncodingException;
	...
	public static FieldParseInfo getInstance(IsoType t, int len, String encoding) {
		...
		if (fpi == null) {
			throw new IllegalArgumentException(String.format(
					"Cannot parse type %s", t));
		}
		fpi.setCharacterEncoding(encoding);
		return fpi;
	}

	protected int decodeLength(byte[] buf, int pos, int digits)
