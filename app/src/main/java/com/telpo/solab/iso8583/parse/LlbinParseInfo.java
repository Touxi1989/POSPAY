...
public class LlbinParseInfo extends FieldParseInfo {
	...
	public <T> IsoValue<?> parse(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid LLBIN field %d position %d", field, pos), pos);
		} else if (pos + 2 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient LLBIN header field %d", field), pos);
		}
		length = decodeLength(buf, pos, 2);
		if (length < 0) {
			throw new ParseException(String.format(
					"Invalid LLBIN field %d length %d pos %d", field, length,
					pos), pos);
		}
	...
	public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid bin LLBIN field %d position %d", field, pos), pos);
		} else if (pos + 1 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient bin LLBIN header field %d", field), pos);
		}
		// final int l = (((buf[pos] & 0xf0) >> 4) * 10) + (buf[pos] & 0x0f);
		// luyq 取模求长度 发现长度多了一倍
		int l = (((buf[pos] & 0xf0) >> 4) * 10) + (buf[pos] & 0x0f);
		l = l / 2 + (l % 2);

		if (l < 0) {
			throw new ParseException(String.format(
					"Invalid bin LLBIN length %d pos %d", length, pos), pos);
		}
