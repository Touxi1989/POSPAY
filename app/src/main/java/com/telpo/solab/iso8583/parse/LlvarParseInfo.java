...
public class LlvarParseInfo extends FieldParseInfo {
    ...
    public <T> IsoValue<?> parse(final int field, final byte[] buf,
        ...
        if (pos < 0) {
            throw new ParseException(String.format("Invalid LLVAR field %d %d",
                    field, pos), pos);
        ...
        if (length < 0) {
            throw new ParseException(String.format(
                    "Invalid LLVAR length %d, field %d pos %d", length, field,
                    pos), pos);
        } else if (length + pos + 2 > buf.length) {
            throw new ParseException(
                    String.format(
                            "Insufficient data for LLVAR field %d, pos %d",
                            field, pos), pos);
        }
        String _v;
        try {
            _v = length == 0 ? "" : new String(buf, pos + 2, length,
                    getCharacterEncoding());
        } catch (IndexOutOfBoundsException ex) {
            throw new ParseException(String.format(
                    "Insufficient data for LLVAR header, field %d pos %d",
                    field, pos), pos);
        }
    ...
    public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
        ...
        if (pos < 0) {
            throw new ParseException(String.format(
                    "Invalid bin LLVAR field %d pos %d", field, pos), pos);
        ...
        if (length < 0) {
            throw new ParseException(String.format(
                    "Invalid bin LLVAR length %d, field %d pos %d", length,
                    field, pos), pos);
        }
        if (length + pos + 1 > buf.length) {
            throw new ParseException(String.format(
                    "Insufficient data for bin LLVAR field %d, pos %d", field,
                    pos), pos);
        }
        if (custom == null) {
            ...
            //name= "ascii";
            name = "gbk";
            return new IsoValue<String>(type, new String(buf, pos + 1, length, name), null);
        } else {
            T dec = custom.decodeField(new String(buf, pos + 1, length,
                    getCharacterEncoding()));
            return dec == null ? new IsoValue<String>(type, new String(buf,
                    pos + 1, length, getCharacterEncoding()), null)
